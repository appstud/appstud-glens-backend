...
def draw_data(img,faceROIs,data,landmarks, intrinsicMatrix, extrinsicMatrix,FPSFaceDetection,FPSWithAgeSexGlasses,FPSWithBeard, FPSWithPose,FPSWithHairColor,draw_weak_perspective=False):
    ...
    for i, d in enumerate(data):
        ...
        if(not draw_weak_perspective):
            img,_=drawCoordinateSystems(intrinsicMatrix, np.array(extrinsicMatrix[i]), img, _3Dpoints=np.array([[0,0,0],[0.02, 0, 0], [0, 0.02, 0], [0, 0, -0.3]]))
        else:
            img=draw_axis(img, d[5][0], d[5][1], d[5][2], tdx=f[0]+f[2]/2, tdy=f[1]+f[3]/2, size = 150)
        ...
        """
        cv2.putText(img,'age:'+str(int(d[0])), (f[0]+f[2],f[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), lineType=cv2.LINE_AA)
        cv2.putText(img,'sex:'+d[1], (f[0]+f[2],f[1]+20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), lineType=cv2.LINE_AA)
        
        cv2.putText(img,'glasses:'+d[2], (f[0]+f[2],f[1]+40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), lineType=cv2.LINE_AA)
                 
        cv2.putText(img,'pose:'+str(list(map("{:.3f}".format,d[5]))), (f[0]+f[2],f[1]+60), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), lineType=cv2.LINE_AA)
        cv2.putText(img,'hair:'+str(d[4]), (f[0]+f[2],f[1]+80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), lineType=cv2.LINE_AA)
        cv2.putText(img,'beard_mask:'+str(d[3]), (f[0]+f[2],f[1]+100), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), lineType=cv2.LINE_AA)
        """
