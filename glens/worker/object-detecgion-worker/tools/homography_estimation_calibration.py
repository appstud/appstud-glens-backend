...
def estimate_homography(cam_id="0"):
    ...
    write_config_to_file(data)
    testExtrinsicParametersCalculation(cam_id)
     

def testExtrinsicParametersCalculation(cam_id):
    data=read_config_from_file('config_oxford.json')
    print(data)
    intrinsicMatrix,_,_=getIntrinsicExtrinsicMatrix(data[cam_id])
    ...
    drawCoordinateSystems(intrinsicMatrix,extrinsicMatrix[0:3,:],img, _3Dpoints=np.array([[0,0,0],[5, 0, 0], [0, 5, 0], [0, 0, 5]]))
    cv2.imshow("img",img)
    cv2.waitKey(100000)
...
if(__name__=="__main__"):
    ...
    testExtrinsicParametersCalculation("0")
    #estimate_homography(img)
