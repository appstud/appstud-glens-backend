...
def draw_data(img, d,ID ,intrinsicMatrix=None,FPS_clustering="notAvailable", FPSFaceDetection="notAvailable",FPS_object_detection="notAvailable",FPS_tracking="notAvailable",FPS_tf_models="notAvailable",draw_weak_perspective=True):

    try:
        cv2.putText(img,'FPS face:'+str(FPSFaceDetection), (5,10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), lineType=cv2.LINE_AA)
        cv2.putText(img,'FPS object detection:'+str(FPS_object_detection), (5,30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), lineType=cv2.LINE_AA)
        cv2.putText(img,'FPS tf models:'+str(FPS_tf_models), (5,50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), lineType=cv2.LINE_AA)
        cv2.putText(img,'FPS tracking:'+str(FPS_tracking), (5,70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), lineType=cv2.LINE_AA)
        cv2.putText(img,'FPS clustering:'+str(FPS_clustering), (5,90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), lineType=cv2.LINE_AA)
    except:
        pass
    
    f=list(map(int,d["bbox"])) if "bbox" in d.keys() else list(map(int,d["bbox_yolo"]))
    #l=landmarks[i]
    try:
    ...
    img=cv2.rectangle(img,(f[0],f[1]),(f[2]+f[0],f[3]+f[1]),(0,255,0),3)   
    img=cv2.circle(img,(int(0.5*f[2]+f[0]),f[3]+f[1]),4,(0,0,255),3) 
    img=cv2.putText(img,'ID:'+str(ID), (f[0],f[1]), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255, 0, 0), lineType=cv2.LINE_AA,thickness=3)
    try:
        if(str(d["beard_mask"])=="mask on"):
            img=cv2.rectangle(img,(f[0],f[1]),(f[2]+f[0],f[3]+f[1]),(0,255,0),1)   
        else:
            img=cv2.rectangle(img,(f[0],f[1]),(f[2]+f[0],f[3]+f[1]),(0,0,255),1)   
        ...
        if(not draw_weak_perspective):
            ...
            img=draw_axis(img, float(d["pose_data"][3]), float(d["pose_data"][4]), float(d["pose_data"][5]), tdx=f[0]+f[2]/2, tdy=f[1]+f[3]/2, size = 150)
        ...
        pass
    try: 
        cv2.putText(img,'age:'+str(int(d["age"])), (f[0]+f[2],f[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), lineType=cv2.LINE_AA)
    except Exception as e:
        pass

    try:
        cv2.putText(img,'sex:'+d["sex"], (f[0]+f[2],f[1]+20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), lineType=cv2.LINE_AA)
    except Exception as e:
        pass

    try:
        cv2.putText(img,'glasses:'+d["glasses"], (f[0]+f[2],f[1]+40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), lineType=cv2.LINE_AA)
    except Exception as e:
        pass
    
    try:
        cv2.putText(img,'pose:'+str(list(map(lambda x:"{:.2f}".format(x) if not isinstance(x,str) else x,d["pose_data"]))), (f[0]+f[2],f[1]+f[3]), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), lineType=cv2.LINE_AA)
    except Exception as e:
        #cv2.putText(img,'pose:'+str(d["pose_data"]), (f[0]+f[2],f[1]+60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), lineType=cv2.LINE_AA)
        pass
    try:
        cv2.putText(img,'hair:'+str(d["hairColor"]), (f[0]+f[2],f[1]+80), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), lineType=cv2.LINE_AA)
    except Exception as e:
        pass
    try:
        cv2.putText(img,'beard_mask:'+str(d["beard_mask"]), (f[0]+f[2],f[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), lineType=cv2.LINE_AA)
    except Exception as e:
        pass
