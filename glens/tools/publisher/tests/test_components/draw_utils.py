...
def draw_data(img, d, intrinsicMatrix=None, FPSFaceDetection="notAvailable",FPSWithAgeSexGlasses="notAvailable",FPSWithBeard="notAvailable", FPSWithPose="notAvailable",FPSWithHairColor="notAvailable",draw_weak_perspective=True):

    cv2.putText(img,'FPS face:'+str(FPSFaceDetection), (5,10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), lineType=cv2.LINE_AA)
    cv2.putText(img,'FPS a_s_g:'+str(FPSWithAgeSexGlasses), (5,30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), lineType=cv2.LINE_AA)
    cv2.putText(img,'FPS beard:'+str(FPSWithBeard), (5,50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), lineType=cv2.LINE_AA)
    cv2.putText(img,'FPS hair:'+str(FPSWithHairColor), (5,70), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), lineType=cv2.LINE_AA)
    cv2.putText(img,'FPS pose:'+str(FPSWithPose), (5,90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), lineType=cv2.LINE_AA)

    f=list(map(int,d["bbox"]))
    #l=landmarks[i]
    ...
    if(str(d["beard_mask"])=="mask on"):
        ...
        img=cv2.rectangle(img,(f[0],f[1]),(f[2]+f[0],f[3]+f[1]),(0,0,255),3)   
    ...
    try:
        cv2.putText(img,'pose:'+str(list(map(lambda x:"{:.2f}".format(x) if not isinstance(x,str) else x,d["pose_data"]))), (f[0]+f[2],f[1]+60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), lineType=cv2.LINE_AA)
    except Exception as e:
