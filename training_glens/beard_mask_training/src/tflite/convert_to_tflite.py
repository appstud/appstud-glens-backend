...
INPUT_RESOLUTION=(96,96,3)

def getModel(weights_path):
    
    model=createModel(training=False)
    model.load_weights(weights_path)
    
    inp = tf.keras.layers.Input(shape=INPUT_RESOLUTION)
    x=rgb_to_bgr_layer=rgb_to_bgr=tf.keras.layers.Lambda(lambda x: tf.reverse(x,[3]))(inp)
...
def convert_to_tflite(model_name,weights_path):
    ...
    input_arr = 255*np.ones((1,96, 96,3))
...
def test_tflite_model(model_path,source=0):
    ...
    while(True):
        ...
        finish=time.time()
        try:
            croppedFace=cv2.resize(img[bbox[0][1]:bbox[0][3]+bbox[0][1], bbox[0][0]:bbox[0][2]+bbox[0][0],:],(96,96))
...
if(__name__=="__main__"):
    convert_to_tflite("Model_mask","./weights_cbam_shallow.checkpoint")
    test_tflite_model(model_path="./Model_mask.tflite",source=0)
